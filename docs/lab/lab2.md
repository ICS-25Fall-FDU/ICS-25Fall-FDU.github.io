...
这是一款全新的代码世界冒险游戏，相比 csapp 的经典款BombLab有着更高的可玩性。在本代码世界冒险游戏中，你将扮演一位精通汇编语言/ linux / gdb / 数据结构的幸存者，在一座即将走向末路的“二进制城市”中，通过自己的智慧与胆量发现隐藏在混乱的 x86 汇编背后的真相......
...
GDB 作为调试器是在你的程序跑起来之后才发挥主要作用的。用法可以参考`gdb-tutor`实操练习，也可以自行上网查阅。

你可以通过命令 `man gdb` 或者上网搜索来获得其更详尽的用法。以下为一些 gdb 的常用指令：
...
如前文所述，二进制炸弹bomb++是一个由二进制**机器码**组成的可执行文件，因此我们无法指望直接阅读它来拆除炸弹。但我们可以将可执行文件中的机器码转化为汇编代码后再进行阅读。
...
以上命令会将 `phase_5` 这一函数的汇编代码放入 `phase_5.S` 中。
...
在拆弹过程中，如果你已经将所有正确口令保存在 `password.txt` 中，可以这样一次性通过所有关卡：
...
如果你已经知道前几个阶段的口令并保存在 `password.txt` 中，但不想在后续阶段中一遍遍手动输入，就可以用 `cat` 打开那个文本文件以及标准输入，然后利用管道机制将这两个文件的内容导向bomb+的输入：
...
>
>   3. 密码就在你一路走来的路上，请仔细关注每一道关卡中出现的数字，它可能在你的**答案**里，也可能在某个**寄存器**里。 别忘了**全局变量**！！！ 
>
