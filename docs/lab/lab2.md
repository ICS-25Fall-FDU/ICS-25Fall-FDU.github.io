...
这是一款全新的代码世界冒险游戏，相比 CSAPP 的经典款 BombLab 有着更高的可玩性。在这座即将走向末路的“二进制城市”中，你将扮演一位精通汇编语言 / linux / GDB / 数据结构的幸存者，通过自己的智慧与胆量发现隐藏在混乱的 x86 汇编背后的真相……
...
本实验满分为 110 分（但是计入 Lab 成绩的上限为 100 分）。每迟交一天（迟交 1ms 到 24h 均视作一天）会在你的得分上扣除 5 分，扣完为止。例如：如果你迟交一天，但是完成了 107 分的作业，则你的实验得分为 100 分。
